...
import "./DirectoryOrderCard.css";

export default function DirectoryOrderCard({ orderId }) {
  ...
  return (
    <>
      <div className="directory-card-title">{order.title}</div>
      <div className="directory-card-bottom">
        <span className="directory-card-type">{order.type}</span>
        <span className="directory-card-date">{order.date}</span>
        <span className={`directory-card-status status-${order.status.replace(" ", "-")}`}>
          {order.status}
        </span>
      </div>
    </>
