...
const RequestPostForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'requestPosts'), {
        ...formData,
  ...
  return submitted ? (
    ...
    <form className="request-post-form" onSubmit={handleSubmit}>
      {/* Title */}
      <label className="request-form-label">Title</label>
      <input
        type="text"
        name="title"
      ...
      {/* Toggle Buttons */}
      <div className="request-toggle-buttons-row">
        <img
      ...
      <input
        ...
        onChange={handleChange}
        className={activeField === 'address' ? '' : 'request-hidden-input'}
      />
      ...
      <input
        ...
        value={formData.link}
        onChange={handleChange}
        className={activeField === 'link' ? '' : 'request-hidden-input'}
      />

      {/* Image Upload */}
      {activeField === 'image' && (
        <div className="request-upload-container">
          <label className="request-form-label">Upload Image</label>
          <input
            type="file"
            accept="image/*"
            onChange={handleFileUpload}
          />
          {uploading && <p className="request-uploading-text">Uploading image...</p>}
          {formData.image && (
            <img src={formData.image} alt="Uploaded" className="request-preview-media" />
          )}
        </div>
      )}
      ...
      <input
        ...
        onChange={handleChange}
        className={activeField === 'tokens' ? '' : 'request-hidden-input'}
      />
      ...
      {activeField === 'urgency' && (
        <select
          ...
          onChange={handleChange}
          className="request-urgency-select"
        >
      ...
      <button type="submit" className="request-submit-btn">Submit</button>
