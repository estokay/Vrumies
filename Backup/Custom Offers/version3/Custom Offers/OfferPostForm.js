...
const OfferPostForm = () => {
  ...
  const MAX_IMAGES = 7;
  const GOOGLE_API_KEY = 'AIzaSyAF-Y9M2YTMmMBu6RU7sDh4vRM9gFdC5MI'; // Replace with your key
  ...
  return submitted ? (
    ...
    <form className="mpf-post-form" onSubmit={handleSubmit}>
      ...
      <input type="text" name="link" placeholder="Enter link" value={formData.link} onChange={handleChange} className={activeField === 'link' ? '' : 'mpf-hidden-input'} />
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '10px' }}>
            {formData.images.map((img, index) => (
              <div key={index} style={{ position: 'relative' }}>
                ...
                <button type="button" style={{ position: 'absolute', top: '-8px', right: '-8px', background: 'red', color: '#fff', border: 'none', borderRadius: '50%', width: '20px', height: '20px', cursor: 'pointer' }} onClick={() => handleRemoveImage(index)}>×</button>
              </div>
            ))}
          </div>
        </div>
      ...
      <input type="number" name="tokens" placeholder="Enter token amount" value={formData.tokens} onChange={handleChange} className={activeField === 'tokens' ? '' : 'mpf-hidden-input'} />
