...
function PostSection({ postId }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Report check (array of user IDs, backward-compatible)
        if (currentUser) {
  ...
  // Backward-compatible report function using array of user IDs
    ...
    try {
      ...
      if (reportArray.includes(currentUser.uid)) {
        await updateDoc(postRef, {
          report: arrayRemove(currentUser.uid) // safe even if array doesn't exist
        });
        setReported(false);
        showNotification("Report removed");
      } else {
        await updateDoc(postRef, {
          report: arrayUnion(currentUser.uid) // creates array if it doesn't exist
        });
        setReported(true);
