...
function PostSection({ postId }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Set initial bookmark and report status
        if (currentUser) {
  ...
  const handleReport = async () => {
    ...
    try {
      ...
      showNotification("Post report " + (!reported ? "submitted" : "removed"));
    } catch (err) {
    ...
    <div className="post-section">
      ...
      <div className="post-left">
        <div className="post-content">
          ...
          {activeTab === "reviews" && <div className="seller-reviews-section"><p>Seller reviews will go here...</p></div>}
        </div>

        {/* Bottom-left / bottom-right buttons */}
        <div className="bottom-left-actions">
          <div className="actions">
            <button className={`action-btn ${bookmarked ? "active" : ""}`} onClick={handleShare}>
            ...
            <button className={`action-btn ${reported ? "active" : ""}`} onClick={handleReport}>
              <FaFlag color="#00ff00" />
              REPORT
            </button>
          </div>
      ...
      <div className="post-right">
        ...
        <div className="carousel">
          ...
          {images.map((img, idx) => (
            <img key={idx} src={img} alt={`Thumb ${idx}`} className={`thumb ${idx === currentImage ? "active" : ""}`} onClick={() => setCurrentImage(idx)} />
          ))}
