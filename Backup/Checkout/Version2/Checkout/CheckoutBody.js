...
const CheckoutBody = forwardRef(() => {
  ...
  // Fetch cart items for subtotal/total
  useEffect(() => {
  ...
  // Auto-copy shipping to billing if checkbox checked
  useEffect(() => {
  ...
  const submitPayment = async () => {
    ...
    if (error) {
      ...
      // TODO: send paymentMethod.id + shipping info + total to backend
    }
  };
...
const cardStyle = {
  style: {
    base: { color: "#ffffff", fontSize: "16px", "::placeholder": { color: "#bbb" } },
