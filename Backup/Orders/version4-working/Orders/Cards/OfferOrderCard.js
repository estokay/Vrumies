...
export default function OfferOrderCard({ orderId }) {
  ...
  return (
    <>
      <div className="event-card-title">{order.title}</div>
      <div className="event-card-bottom">
        <span className="event-card-type">{order.type}</span>
        <span className="event-card-date">{order.date}</span>
        <span className={`event-card-status status-${order.status.replace(" ", "-")}`}>
          {order.status}
        </span>
      </div>
    </>
